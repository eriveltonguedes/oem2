...
Ecint <- dados.reais.lista[["Educacao.CI.e"]]
...
Ecint_novos <- Ecint[Ecint$Cod.IBGE %in% municipios_novos_naousar[-11],]
Ecint_novos <- Ecint_novos[order(Ecint_novos$Cod.IBGE),]
Ano <- c("2013","2008","2009","2013","2013","2013","2008","2013",
         "2008","2008")
Ecint_novos[is.na(Ecint_novos)] <- 0

for (i in 1:dim(Ecint_novos)[1]) {
  ass <- Ano[i]
  cols <- ass:2019
  cols2 <- c("Cod.IBGE", cols)
  dados <- Ecint_novos[i,colnames(Ecint_novos) %in%cols]
  dados[dados==0] <- NA
  Ecint_novos[i,colnames(Ecint_novos) %in%cols] <- dados
  Ecint_novos[i,!(colnames(Ecint_novos) %in% cols2)] <- 0
  
}

Ecint <- Ecint[!(Ecint$Cod.IBGE %in% municipios_novos_naousar),]
Ecint[Ecint==0] <- NA
...
nomes <- c("Cod.IBGE","Ecint0809","Ecint0910", "Ecint1011", "Ecint1112",
           "Ecint1213", "Ecint1314", "Ecint1415","Ecint1516", "Ecint1617",
           "Ecint1718", "Ecint1819")
...
Ecint_out_var1 <- join(melt_variacaoreal_abs_ano1, melt(dados, id.vars = "Cod.IBGE"),
                       by=c("Cod.IBGE", "variable"), type="left")

Ecint_out_var2 <- join(melt_variacaoreal_abs_ano2, melt(dados, id.vars = "Cod.IBGE"),
                       by=c("Cod.IBGE", "variable"), type="left")

Ecint_out_var <- join(Ecint_out_var1, Ecint_out_var2,  by=c("Cod.IBGE", "variable"), type="full")
colnames(Ecint_out_var)[2] <- "Ano"

Ecint_out_vnv <- Ecint_out_var

###### Outliers
Ecint_outliers <- join(Ecint_out_rnv, Ecint_out_vnv, by=c("Cod.IBGE","Ano"), type="full")

Ecint_comp_p_imput <- dados
for (j in 1:dim(Ecint_outliers)[1]) {
  mun <- as.numeric(as.vector(Ecint_outliers[j,1]))
  ano <- as.numeric(as.vector(Ecint_outliers[j,2]))
  col <- ano-2006
  Ecint_comp_p_imput[Ecint_comp_p_imput$Cod.IBGE == mun, col] <- NA
}

Ecint_comp_p_imput2 <- Ecint_comp_p_imput


##### IMPUT 2 #####
dados <- Ecint_comp_p_imput2
dados[dados==0] <- NA
dados_semna <- na.omit(dados)
dados_comna <- subset(dados, !Cod.IBGE %in% dados_semna$Cod.IBGE)
a <- list()
...
dados_imputada1 <- rbind(dados_semna,dados_imput1)
Ecint_imput1.n <- dados_imputada1
...
#### DADOS FINAIS ####
Ecint_bruta <- rbind(Ecint, Ecint_novos)
Ecint_bruta <- Ecint_bruta[order(Ecint_bruta$Cod.IBGE),]
save(Ecint_bruta, file = "data/Ecint_bruta.RData")


Ecint_imput_missing <- rbind(Ecint_imput1, Ecint_imput1.n)
Ecint_imput_missing <- Ecint_imput_missing[order(Ecint_imput_missing$Cod.IBGE),]
save(Ecint_imput_missing, file = "data/Ecint_imput_missing.RData")


Ecint_outliers <- Ecint_outliers
colnames(Ecint_outliers)[3] <- "Valor"
Ecint_outliers <- Ecint_outliers[order(Ecint_outliers$Cod.IBGE,
                                       Ecint_outliers$Ano),]
save(Ecint_outliers, file = "data/Ecint_outliers.RData")


Ecint_final <- rbind(Ecint_imput2, Ecint_imput1.n)
Ecint_final <- Ecint_final[order(Ecint_final$Cod.IBGE),]
save(Ecint_final, file = "data/Ecint_final.RData")
